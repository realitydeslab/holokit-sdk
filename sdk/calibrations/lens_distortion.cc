...
LensDistortion::LensDistortion(const DeviceParams& device_params,
                               int display_width, int display_height) {
    
  device_params_ = device_params;
  eye_from_head_matrix_[kLeft] = holokit::Matrix4x4::Translation(
      device_params_.inter_lens_distance * 0.5f, 0.f, 0.f);
  eye_from_head_matrix_[kRight] = holokit::Matrix4x4::Translation(
      -device_params_.inter_lens_distance * 0.5f, 0.f, 0.f);

  distortion_ = std::unique_ptr<PolynomialRadialDistortion>(
      new PolynomialRadialDistortion(device_params_.distortion_coefficients));
...
//  }
    return screen_height_meters - device_params.tray_to_lens_distance -             kDefaultBorderSizeMeters;
}

DistortionMesh* LensDistortion::CreateDistortionMesh(
    HoloKitEye eye, const DeviceParams& device_params,
