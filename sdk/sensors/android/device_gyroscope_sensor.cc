...
const ASensor* InitSensor(ASensorManager* sensor_manager) {
  ...
  if (gyro != nullptr) {
    HOLOKIT_LOGI("Android Gyro Sensor: ASENSOR_TYPE_GYRO_UNCALIBRATED");
    return gyro;
  }
  HOLOKIT_LOGI("Android Gyro Sensor: ASENSOR_TYPE_GYROSCOPE");
  return ASensorManager_getDefaultSensor(sensor_manager,
...
bool ParseGyroEvent(const ASensorEvent& event,
  ...
  if (event.type == ASENSOR_TYPE_ADDITIONAL_INFO) {
    HOLOKIT_LOGI("ParseGyroEvent discarding additional info sensor event");
    return false;
  }
  ...
  if (event.type == ASENSOR_TYPE_GYROSCOPE) {
    ...
    if (sensor_info->first_gyro_value) {
      ...
      HOLOKIT_LOGI("Android gyro bias is: %f, %f, %f",
    ...
    HOLOKIT_LOGE("ParseGyroEvent discarding unexpected sensor event type %d",
                   event.type);
  }
...
bool DeviceGyroscopeSensor::Start() {
  if (!sensor_info_->reader) {
    HOLOKIT_LOGE("Could not start gyroscope sensor.");
    return false;
  }
