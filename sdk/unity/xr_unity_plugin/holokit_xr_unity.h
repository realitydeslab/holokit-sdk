 ...
 */
#ifndef HOLOKIT_SDK_UNITY_XR_UNITY_PLUGIN_HOLOKIT_XR_UNITY_H_
#define HOLOKIT_SDK_UNITY_XR_UNITY_PLUGIN_HOLOKIT_XR_UNITY_H_
...
/// Minimalistic wrapper of HoloKit SDK with native and standard types which
/// hides HoloKit types and exposes enough functionality to be consumed by a
/// XR Unity plugin.
///
/// HoloKit SDK interface has C-structs and native types but its use requires
...
/// specific calls and HoloKit SDK configurations reducing them to a minimum.
class HoloKitApi {
 public:
  ...
  ///          HoloKit V1 device parameters is used.
  ...
  /// @brief Render the HoloKit widgets (X, Gear, divider line) to the screen.
  /// @pre It must be called from the rendering thread.
  void RenderWidgets();
 ...
 private:
  ...
  class HoloKitApiImpl;

  // @brief Implementation of this class that hides HoloKit types.
  std::unique_ptr<HoloKitApiImpl> p_impl_;
};
...
///          HoloKitXRLoader::Initialize() from C# code when loading the
///          provider. Provided parameters will be returned by
///          HoloKitApi::GetScreenParams().
/// @param[in] x x coordinate in pixels of the lower left corner of the
///            rectangle.
/// @param[in] y y coordinate in pixels of the lower left corner of the
///            rectangle.
...
#endif  // HOLOKIT_SDK_UNITY_XR_UNITY_PLUGIN_HOLOKIT_XR_UNITY_H_
