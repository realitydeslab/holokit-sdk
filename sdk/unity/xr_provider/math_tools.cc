...
static UnityXRVector4 HoloKitTransformToUnityQuat(
    const std::array<float, 16>& transform_in) {
  // HoloKit matrices are row major, unity (and this algorithm) is column
...
UnityXRPose HoloKitTransformToUnityPose(
  ...
  UnityXRPose ret;
  ret.rotation = HoloKitTransformToUnityQuat(transform);

  // HoloKit matrices are transforms into view space, unity wants transforms
