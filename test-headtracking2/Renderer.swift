...
class Renderer {
    ...
    var viewportPerEye: [MTLViewport]?
    ...
    func loadMetal() {
        ...
        anchorPipelineStateDescriptor.maxVertexAmplificationCount = 2 //Added
    ...
    func loadAssets() {
        ...
        let mesh = MDLMesh(boxWithExtent: vector3(0.0075, 0.0075, 0.0075), segments: vector3(1, 1, 1), inwardNormals: false, geometryType: .triangles, allocator: metalAllocator)
    ...
    func updateAnchors(frame: ARFrame) {
        ...
        for index in 0..<anchorInstanceCount {
            ...
            if anchor.name?.starts(with: "handtracking") ?? false {
                anchorUniforms.pointee.anchorColor = simd_float4(1.0, 1.0, 0.5, 1.0)
            } else if anchor.name?.starts(with: "nearbyinteraction") ?? false {
                anchorUniforms.pointee.anchorColor = simd_float4(1.0, 0.5, 1.0, 1.0)
            } else {
                anchorUniforms.pointee.anchorColor = simd_float4(1.0, 1.0, 1.0, 1.0)
            }
        }
    }
