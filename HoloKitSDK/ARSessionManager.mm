...
- (void)session:(ARSession *)session didUpdateFrame:(ARFrame *)frame {
    ...
    // Perform hand tracking algorithm
...
void HoloKitSDK_RegisterARSessionDelegates(void (*OnARSessionUpdatedFrameDelegate)(double, const float *),
                                           void (*OnCameraChangedTrackingStateDelegate)(int),
                                           void (*OnRelocalizationSucceededDelegate)(void)) {
    [ARSessionManager sharedInstance];
...
void HoloKitSDK_InterceptUnityARSessionDelegates(UnityXRNativeSession* nativeARSessionPtr) {
    ...
    ARSessionManager* arSessionDelegateManager = [ARSessionManager sharedInstance];
...
void HoloKitSDK_SetSessionShouldAttemptRelocalization(bool value) {
    [[ARSessionManager sharedInstance] setSessionShouldAttemptRelocalization:value];
}

void HoloKitSDK_SetBackgroundVideoFormat(int videoFormat) {
    [[ARSessionManager sharedInstance] setDesiredBackgroundVideoFormat:(BackgroundVideoFormat)videoFormat];
}
...
void HoloKitSDK_ResetOrigin(float position[3], float rotation[4]) {
    ...
    [[[ARSessionManager sharedInstance] arSession] setWorldOrigin:transform_matrix];
}

void HoloKitSDK_PauseCurrentARSession(void) {
    [[ARSessionManager sharedInstance] pauseCurrentARSession];
}

void HoloKitSDK_ResumeCurrentARSession(void) {
    [[ARSessionManager sharedInstance] resumeCurrentARSession];
}
